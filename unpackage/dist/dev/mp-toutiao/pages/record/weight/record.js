...
const _sfc_main = {
  ...
  methods: {
    ...
    async init() {
      ...
      let res = await common_api_api.Api.getMyWeightRecords();
      res = res.sort((a, b) => new Date(b.createTime) - new Date(a.createTime));
      this.weightRecords = res;
      this.defaultValue = this.inputWeight = Number(((_a = res[0]) == null ? void 0 : _a.weight) || 70);
      setTimeout(() => {
        this.loadChart();
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    a: common_vendor.t($data.inputWeight),
    b: $data.inputWeight
  }, $data.inputWeight ? {
    c: common_vendor.sr("slide", "bf75bb63-0"),
    ...
    g: $data.weightRecords.length
  }, $data.weightRecords.length ? {
    h: common_vendor.sr("chartRef", "bf75bb63-1")
  } : {}, {
    i: $data.weightRecords.length
  }, $data.weightRecords.length ? {
    j: common_vendor.f($data.weightRecords, (record, index, i0) => {
      return {
        a: common_vendor.t(record.weight),
        b: common_vendor.o(($event) => $options.deleteRecord(record)),
        c: "bf75bb63-4-" + i0 + "," + ("bf75bb63-3-" + i0),
        d: "bf75bb63-3-" + i0 + ",bf75bb63-2",
        e: common_vendor.p({
...
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["render", _sfc_render], ["__scopeId", "data-v-bf75bb63"]]);
